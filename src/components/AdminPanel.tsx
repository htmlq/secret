...
export default function AdminPanel() {
  ...
  useEffect(() => {
    if (username) {
      const exists = usernameList.some(u => u.username.toLowerCase() === username.toLowerCase());
    ...
    <div className="space-y-8">
      <div className="w-80 mx-auto bg-gray-800 rounded-lg p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="relative">
              ...
              {usernameStatus && (
                <span className={`absolute right-3 top-1/2 -translate-y-1/2 text-xs ${
                  ...
                  {usernameStatus === 'available' ? 'Can add' : 'Already listed'}
                </span>
              )}
            </div>
          </div>
          ...
          <div>
            <label className="block text-gray-300 mb-2">Features (3 required)</label>
      ...
      <div className="w-80 mx-auto bg-gray-800 rounded-lg p-6">
        <h2 className="text-xl font-bold text-white mb-6 text-center">Current Usernames</h2>
